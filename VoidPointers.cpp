...
void typeCastToAllDataTypes(void * long_ptr, void * float_ptr, void * char_ptr, void * str_ptr) {
	...
	printf("Value stored at char_ptr is %ld\n", *(char *)long_ptr);
	printf("Value stored at str_ptr is %ld\n", *(char* *)long_ptr);
	//Other code
}
...
void task7() {
	...
	void * f=getFloatAsVoidStar();
	void * i = getIntAsVoidStar();
	void * s = getStringAsVoidStar();
	printf("Float is %f", *(int *)f);
	printf("Int is %d", *(char *)f);
	printf("String is %s", *(char* *)f);
}
...
void ** getTwoGreater(void *addr1, void *addr2, void *addr3, int(*compare)(void *, void *)) {
	...
	int k1=compare(addr1, addr2);
	if (k1 == 1){
		k1 = compare(addr1, addr3);
		if (k1 == 1){
			result[0] = addr1;
			k1 = compare(addr2, addr3);
			if (k1 == 1){
				result[1] = addr2;
			}
			else{
